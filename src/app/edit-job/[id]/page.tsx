...
function EditJobPage() {
  ...
  ); // Ensure 'jobAction' is the correct key
  ...
  const loadJobData = useCallback(async () => {
    ...
    try {
      ...
      if (fetchedJob.location) {
        dispatch(
          updateDropdownContent({ id: 0, content: fetchedJob.location })
        );
      }
      if (fetchedJob.experience) {
        dispatch(
          updateDropdownContent({ id: 1, content: fetchedJob.experience })
        );
      }
    } catch (fetchError) {
